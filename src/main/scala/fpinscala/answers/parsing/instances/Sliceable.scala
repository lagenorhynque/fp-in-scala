...
object Sliceable extends Parsers[Sliceable.Parser]:
  ...
  /** The result of a parse--a `Parser[A]` returns a `Result[A]`.
    ...
    *
    * `Result` is an example of a Generalized Algebraic Data Type (GADT), which
    * means that not all the data constructors of `Result` have the same type.
    * In particular, `Slice` _refines_ the `A` type parameter to be `String`. If
    * we pattern match on a `Result` and obtain a `Slice`, we expect to be able
    * to assume that `A` was in fact `String` and use this type information
    * elsewhere.
    */
  enum Result[+A]:
  ...
  /** Returns -1 if s.startsWith(s2), otherwise returns the first index where
    * the two strings differed. If s2 is longer than s1, returns s.length.
    */
